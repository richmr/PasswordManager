...
function getSalt(size=12) {
  // Returns a Uint8Array salt
  var iv = window.crypto.getRandomValues(new Uint8Array(size));
  return iv;
}
...
async function generateAndExportKey(key = null) {
  // If no CryptKey is provided then generates a random, new symmetric key for Subtle Crypt
  // Then exports the key material as a Uint8Array RAW key
  if (key == null) {
    key = await generateSymKey();
  }
  var keymat = await crypto.subtle.exportKey("raw", key);
  // Convert to Uint8Array
  var key_arr = new Uint8Array(keymat);
  return key_arr;
}

async function importRawKey(raw_keymat) {
  // Returns a cryptokey from raw material
  // raw_keymat can be a B64string or an array
  if (typeof raw_keymat == "string") {
    raw_keymat = base64StringToUInt8Arr(raw_keymat);
  }
...
/**************
 EZ Encrypt functions
 These use base64 cipher strings of the form:
...
async function ezSubtleEncrypt(data, key) {
  ...
  // encrypt it
  var ct = await encryptData(key, iv, data);
...
async function ezSubtleDecrypt(cipher_data, key) {
  ...
  // decrypt it
  var pt = await decryptData(key, iv_b64, ct_b64);
...
/****************
Basic Subtle crypto wrappers
Subtle crypto has a lot of "fiddly" bits.  These wrappers
abstract this.
Please note they also ONLY provide symmetric encryption at the moment.
They accept a variety of input forms, but only return results as Uint8Array
*****************/
